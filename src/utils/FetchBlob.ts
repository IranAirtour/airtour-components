...
import { JwtServiceClient } from '../network/JwtService';
...
import { IServerAttachment } from '../models/ApiModels/Attachment';
import { MessageFactory } from '../Factory/Message';
import { logDebug, logError } from './Logger';
import { jsonParse } from './StringUtils';
...
export class FetchBlobHandler implements IFetchBlobHandler {
  ...
  async downloadManager(
    ...
    try {
      const AccessToken =
        await JwtServiceClient.getAccessTokenWithRefreshHandling();
      const fileName =
  ...
  async uploadManager(
    ...
    try {
      ...
      const url = MessageFactory.generateAttachmentUrl(responseData);
